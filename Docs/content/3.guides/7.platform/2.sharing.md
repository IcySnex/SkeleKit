...
`ISharer` is registered with the other built-in services described in [application setup and dependency injection](/guides/application/application-builder-and-dependency-injection#services), but can also be accessed from the view directly using `ContentView.Sharer`.
