...
`AccessibilityTrait` is a flags enum assigned through `AccessibilityTraits`. It can combine roles such as `Button`, `Link`, `Header`, `Image`, `Selected`, `StaticText`, or `Adjustable`. Use the semantic role the element actually performs.
...
See [colors and materials](/guides/styling-and-motion/colors-and-materials#semantic-colors) and [application setup and DI](/guides/application/application-setup-and-di#appearance) for the related visual behavior.
