...
`IHaptics` is registered with the other built-in services described in [application setup and dependency injection](/guides/application/application-builder-and-dependency-injection#services). Custom views can use the same service directly through their protected `Haptics` property.
...
For the command and gesture boundaries that usually trigger feedback, see [commands, menus, and gestures](/guides/foundations/commands-menus-and-gestures).
