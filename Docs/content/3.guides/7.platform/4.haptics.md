...
`IHaptics` is registered with the other built-in services described in [application setup and dependency injection](/guides/application/application-builder-and-dependency-injection#services). Page code can use the same service directly through `ContentView.Haptics`.
