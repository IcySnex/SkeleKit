...
For a converted two-way binding, add the source writer and then convert incoming control values back to the source type:
...
The writer receives the source type. In this example, `age` is an `int` even though the control exposes a nullable string. A converted `TwoWay` chain is incomplete until `ConvertFrom` is supplied, so it cannot be assigned accidentally without the return conversion.
...
```csharp
Text = Bind()
    .ToSource<string>((vm, value) => vm.Draft = value ?? string.Empty);
```
...
Text = Bind(vm => vm.Movie)
    .Path(movie => movie.Name)
    .TwoWay((movie, name) => movie.Name = name ?? string.Empty);
```
