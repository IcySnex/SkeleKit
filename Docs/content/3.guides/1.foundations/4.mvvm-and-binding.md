...
Adapters can deconstruct an existing two-way binding into its typed path and writer, then recompose it through the same conversion pipeline:

```csharp
var (expression, write) = binding;

BindingExpression<string?> adapted = expression
    .ConvertTo(value => LocalizedTitle(value))
    .ConvertFrom(title => ValueFor(title))
    .TwoWay(write);
```
...
`BindableList<T>` exposes `Count`, indexing, enumeration, and `Add` for a mutable literal source, so construction patterns such as `control.Items.Add("Recent")` remain available. A bound or otherwise read-only source cannot be mutated through the wrapper; mutate the source collection instead.
...
Brush-valued properties such as `Background` accept the same direct values as before—colors, gradients, materials, and `null`—and also accept bindings. A color binding is promoted to a solid brush automatically:

```csharp
Background = Bind(vm => vm.CardColor);
```

Changes to the source update the existing view, including a bound value becoming `null`.
...
When a property should observe a specific object instead of the surrounding view's context, pass that object to `BindingFactory.Bind`:

```csharp
IsOn = BindingFactory.Bind(config, config => config.NotificationsEnabled)
    .TwoWay((config, enabled) => config.NotificationsEnabled = enabled);
```

The binding keeps `config` as its source through `Path`, conversion, and binding-mode changes. This is useful for reusable item descriptors: the descriptor can carry a `Bindable<T>` without copying the source value or placing a `View` in a ViewModel. Changes made elsewhere to the same observable source still update the control directly.
