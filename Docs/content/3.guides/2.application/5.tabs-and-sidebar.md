...
`LargeTitles()` makes large titles the default for pages in each tab stack. A page with `TitleStyle.Automatic` follows that setting; `TitleStyle.Inline` and `TitleStyle.Large` override it for that page. Without `LargeTitles()`, automatic titles remain inline.
