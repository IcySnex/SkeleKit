...
The destination mappings used here come from [page registration](/guides/application/page-registration-and-lifetimes).
...
See [tabs and iPad layouts](/guides/application/tabs-and-ipad-layouts) for declaring those stacks, iPad-only destinations, and tab accessories.

Tapping the already-selected tab pops to its root when needed, otherwise scrolls the root page's first scrolling view to the top. Set `ContentView.TabReselectedCommand` to replace that behavior.
