...
A toolbar item can show text, an SF Symbol, tint, a command, or a menu of `MenuAction` entries. Primary items receive the platform's emphasized treatment.
...
public SearchView()
{
	...
	SearchScopeIndex = Bind(
    	model => model.Scope,
    	static (model, value) => model.Scope = value);
