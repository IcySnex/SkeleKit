...
A presented page owns a navigation stack. Calls to `PushAsync` and `PopAsync` while it is visible stay inside that modal; use `DismissAsync` to remove the modal itself.
...
On iOS 27, pass a placement before the detents to position the sheet within a wide presenting view:

```csharp
ModalStyle.Sheet(
    SheetPlacement.Trailing,
    Detent.Medium,
    Detent.Large)
```

`Automatic` lets UIKit choose; `Leading` and `Trailing` follow the interface's language direction, and `Center` explicitly centers the sheet. Compact sheets may remain full-width, and earlier iOS versions ignore the placement.
