---
title: Authoring layout panels
description: Implement custom measurement and arrangement when the built-in layouts are not enough.
icon: lucide:ruler
---

Derive from `Panel` for a layout with placement rules that cannot be expressed clearly with `Grid`, `StackPanel`, or `Overlay`. A panel participates in two passes:

1. `MeasureOverride` offers space to each child and returns the panel's desired size.
2. `ArrangeOverride` assigns every child a final rectangle.

`Panel` is specifically the multi-child container and exposes `Children`. Single-child framework types use their semantic slot instead: `Border.Child`, and `ContentView`, `ScrollView`, and `ItemView<T>.Content`.

The shared measurement vocabulary is covered in [sizing and safe areas](/guides/foundations/sizing-and-safe-areas).

This panel keeps two alternative children measured and arranged in the same bounds while selecting which size controls the parent. It is the pattern used by the Gallery's preview/code switcher:

```csharp
public sealed class SwitchPanel : Panel
{
    View selected;

    public SwitchPanel(View first, View second)
    {
        selected = first;
        Children.Add(first);
        Children.Add(second);
    }

    public void Select(View child)
    {
        if (ReferenceEquals(selected, child))
            return;

        selected = child;
        InvalidateMeasure();
    }

    protected override Size MeasureOverride(Size availableSize)
    {
        foreach (View child in Children)
            child.Measure(availableSize);

        return selected.DesiredSize;
    }

    protected override Size ArrangeOverride(Size finalSize)
    {
        var bounds = new Rect(Point.Zero, finalSize);

        foreach (View child in Children)
            child.Arrange(bounds);

        return finalSize;
    }
}
```

`DesiredSize` includes a child's margin. `Arrange` applies that margin and the child's alignment inside the rectangle you provide, so panels should pass the full slot rather than subtracting the margin a second time.

When a custom panel supports its inherited `Padding`, use the protected `ContentInsets` value during measurement and arrangement. It contains the explicit padding plus any UIKit margins selected by `SystemInsetEdges`:

```csharp
protected override Size MeasureOverride(Size availableSize)
{
    Thickness insets = ContentInsets;
    Size inner = availableSize.Deflate(insets);
    // Measure children inside inner...
}
```

Call `InvalidateMeasure` whenever a property changes the desired size or child placement. Purely visual changes such as opacity or color do not need another measure pass.

## Layout rules

Measure children with finite constraints where the layout has a real bound. An infinite dimension means the child may request its natural size; handing infinity through every layer can make scroll content or wrapping ambiguous.

Return a finite desired size unless the parent explicitly supports an open dimension. During arrangement, position every child that was measured, including inactive children that must remain ready for an animated transition. Set `IsVisible` false when a child should leave layout entirely.
