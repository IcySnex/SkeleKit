...
.UseStyles(styles => styles
    .Add(new Style<Label>(label =>
        label.TextColor = Colors.Label))
    .Add(CardStyle))
```

When a view is constructed, the registered styles apply from base types to the concrete type. A `Style<View>` can establish shared defaults, then a `Style<Button>` or `Style<Border>` can refine them.

Explicit styles and local initializer values run after the constructor, so they override the implicit styles. Styles are frozen when the app begins using them and cannot be changed later. Use bindable colors, appearance changes, or explicit state updates for runtime theming rather than trying to register another style set.
