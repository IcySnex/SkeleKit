...
sealed class ContactRow : ItemView<Contact>
{
    ...
    {
        container.Background = contact.IsFavorite
...
`CollectionView` owns the cell's `Item`. It is null only before the template receives its first item; `OnItemChanged` is called only with an assigned item.
