...
`IsEditing` reveals the reorder handle and the selection circles, and suppresses `ItemCommand` so taps select instead of activate. Pair it with `SelectsOnlyWhileEditing = true` when selection should be exclusive to edit mode.

Use an `ObservableCollection<T>` for `SelectedItems` so taps write back, and programmatic collection changes move the native circles. Leaving edit mode keeps the bound selection and its highlight; clear it in your Done command to forget it. See [updates and selection](/guides/collections/updates-and-selection) for single selection and the trailing checkmark.
