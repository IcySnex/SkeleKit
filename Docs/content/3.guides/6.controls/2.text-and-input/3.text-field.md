...
`LeadingIcon` and `TrailingIcon` accept SF Symbols or bundle image sources. The trailing icon shares its native slot with `ClearButton`; when both are set, the icon wins.
