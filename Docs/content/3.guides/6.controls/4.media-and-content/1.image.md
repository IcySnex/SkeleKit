...
Symbol configuration can travel with the source so the same size, weight, scale, and rendering colors are honored by images, buttons, tabs, toolbars, menus, and other local icon slots:

```csharp
var favorite = ImageSource.Symbol(
    "star.fill",
    size: 18,
...
System-owned controls can still constrain the final icon dimensions or tint to match their platform chrome. Properties set directly on an `Image` override the corresponding source configuration.
