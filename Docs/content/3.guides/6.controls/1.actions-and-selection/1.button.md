...
`Icon` accepts a local `ImageSource`: an SF Symbol, bundle asset, or encoded image data. Configure a symbol's size, weight, and scale through `ImageSource.Symbol`. `Subtitle` adds secondary text where the selected native button configuration supports it. Use `IconPlacement` to put the image before, after, above, or below the title; `IconSpacing` controls the gap.

```csharp
Icon = ImageSource.Bundle("brand-mark");
```

Remote URLs are intentionally limited to the full [`Image`](/guides/controls/media-and-content/image) control, which owns asynchronous loading, placeholders, and failure handling.
...
Button menus represent actions: choosing an entry executes its command without changing the button's title. Use [`Picker<TItem>`](/guides/controls/actions-and-selection/picker) when the choices represent selection state.
