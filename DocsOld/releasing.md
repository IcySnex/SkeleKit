...
1. Update `SkeleKitPackageVersion` in `Source/Directory.Build.props`.
2. Update the fallback `SkeleKitPackageVersion` in
   `Source/Samples/SkeleKit.Template/SkeleKit.Template.csproj` to the same value. CI rejects a mismatch.
